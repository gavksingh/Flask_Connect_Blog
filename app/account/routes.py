...
def update_own_acct_picture(id):
    ...
    if request.method == "POST":
        if form.picture.data:
            ...
            # insert a unique id to the filename to make sure there aren't two pictures with the same name:
        ...
        try:
            # save the image to folder and path to user
...
# If this user is an author, the authorship of the post will be transferred to the blog team.
    ...
    if request.method == "POST":
        ...
        if id == 1:
            ...
            try:
                ...
                # if the user has posts, delete them
                if user_at_hand.posts:
                    for post in Blog_Posts.query.filter_by(
                        author_id=user_at_hand.id).all():
                        delete_post(post.id)

                # if user has bookmarks, delete them
                if user_at_hand.bookmarks:
                    update_bookmarks(user_at_hand.id)

                # if user has likes, delete them
                if user_at_hand.likes:
                    update_likes(user_at_hand.id)

                # delete the user picture
                if user_at_hand.picture != "" and user_at_hand.picture != "Picture_default.jpg" and os.path.exists(os.path.join(current_app.config["PROFILE_IMG_FOLDER"], user_at_hand.picture)):
                    os.remove(os.path.join(
                        current_app.config["PROFILE_IMG_FOLDER"], user_at_hand.picture))
                ...
                flash("Account has been successfully deleted. We'll miss you!")
                return redirect(url_for('website.home'))
            except:
                flash("Oops, there's an error deleting your account. Please try again.")
                return redirect(url_for('account.manage_acct'))

    return render_template("account/account_mgmt_delete.html", logged_in=current_user.is_authenticated, user_at_hand=user_at_hand)

# ***********************************************************************************************
# STATISTICS
@account.route("/dashboard/stats", methods=["GET", "POST"])
@login_required
def stats():
    if current_user.type == "admin":
        users = Blog_User.query.all()
        stats = Blog_Stats.query.get(1)
        return render_template("account/stats.html", logged_in=current_user.is_authenticated, stats=stats, users=users)
    else:
        return redirect(url_for('account.dashboard'))
...
# BOOKMARKS
# FAVORITES
@account.route("/dashboard/bookmarks", methods=["GET", "POST"])
@login_required
def bookmarks():
    bookmarks = Blog_Bookmarks.query.filter_by(user_id=current_user.id).all()
    return render_template('account/bookmarks.html', logged_in=current_user.is_authenticated, bookmarks=bookmarks)
